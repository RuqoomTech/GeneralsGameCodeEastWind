...
static void populateLadderListBox()
{
	...
	if (selIndex < 0)
		return;
	selID = static_cast<Int>(GadgetListBoxGetItemIntegerData(listboxLadderSelect, selIndex));
	if (!selID)
		return;
    ...
    case GBM_SELECTED:
		{
			...
			if (controlID == buttonOkID)
			{
				...
				ladderIndex = static_cast<Int>(GadgetListBoxGetItemIntegerData( listboxLadderSelect, selectPos, 0 ));
		...
		case GBM_SELECTED:
			{
				GameWindow *control = (GameWindow *)mData1;
				Int controlID = control->winGetWindowId();
				Int selectedID = static_cast<Int>(reinterpret_cast<std::intptr_t>(window->winGetUserData()));
				if(!selectedID)
					break;
				closeRightClickMenu(window);

		...
		case GLM_SELECTED:
			{
				...
				if ( controlID == GetGameListBoxID() )
				{
					...
					if( rowSelected >= 0 )
					{
						...
						req.stagingRoom.id = static_cast<Int>(GadgetListBoxGetItemIntegerData(control, rowSelected, 0));
		...
		case GBM_SELECTED:
			{
				...
				else if ( controlID == buttonJoinID )
				{
					...
					if (selected >= 0)
					{
						Int selectedID = static_cast<Int>(GadgetListBoxGetItemIntegerData(GetGameListBox(), selected));
						if (selectedID > 0)
						{
		...
		case GCM_SELECTED:
			{
				...
				if( controlID == comboLobbyGroupRoomsID )
				{
					...
					{
						Int groupID;
						groupID = static_cast<Int>(GadgetComboBoxGetItemIntegerData(comboLobbyGroupRooms, rowSelected));
		...
		case GLM_RIGHT_CLICKED:
			{
				...
				else if( controlID == GetGameListBoxID() )
				{
					...
					Int selectedID = static_cast<Int>(GadgetListBoxGetItemIntegerData(control, rc->pos));
					if (selectedID > 0)
					{
						StagingRoomMap *srm = TheGameSpyInfo->getStagingRoomList();
						StagingRoomMap::iterator srmIt = srm->find(selectedID);
						if (srmIt != srm->end())
						{
							...
							if (linfo)
							{
								...
								rcMenu->winSetUserData(reinterpret_cast<void *>(static_cast<std::intptr_t>(selectedID)));
								TheWindowManager->winSetLoneWindow(rcMenu);
							}
						}
					}

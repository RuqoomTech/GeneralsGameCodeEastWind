		...
		case GLM_RIGHT_CLICKED:
			{
				...
				if( controlID == buddyControls.listboxBuddiesID )
				{
					...
					GPProfile profileID = static_cast<GPProfile>(GadgetListBoxGetItemIntegerData(control, rc->pos, 0));
					RCItemType itemType = (RCItemType)static_cast<Int>(GadgetListBoxGetItemIntegerData(control, rc->pos, 1));
			...
			case GEM_EDIT_DONE:
			{
				...
				if (selected >= 0)
				{
					GPProfile selectedProfile = static_cast<GPProfile>(GadgetListBoxGetItemIntegerData(buddyControls.listboxBuddies, selected));
...
void updateBuddyInfo()
{
	...
	GadgetListBoxGetSelected(buddyControls.listboxBuddies, &selected);
	if (selected >= 0)
		selectedProfile = static_cast<GPProfile>(GadgetListBoxGetItemIntegerData(buddyControls.listboxBuddies, selected));
		...
		case GLM_RIGHT_CLICKED:
			{
				...
				if( controlID == listboxIgnoreID )
				{
					...
					GPProfile profileID = static_cast<GPProfile>(GadgetListBoxGetItemIntegerData(control, rc->pos));
					UnicodeString nick = GadgetListBoxGetText(control, rc->pos);
					BuddyInfoMap *buddies = TheGameSpyInfo->getBuddyMap();
					BuddyInfoMap::iterator bIt;
		...
		case GBM_SELECTED:
			{
				...
				else if (controlID == buttonDeleteBuddyID)
				{
					...
					if (selected >= 0)
						selectedName = TheNameKeyGenerator->keyToName((NameKeyType)static_cast<int>(GadgetListBoxGetItemIntegerData(listbox, selected)));
		...
		case GLM_DOUBLE_CLICKED:
			{
				...
				if( controlID == listboxBuddyID )
				{
					...
					if (rowSelected >= 0)
					{
						...
						GPProfile buddyID = static_cast<GPProfile>(GadgetListBoxGetItemIntegerData( listboxWindow, rowSelected, 0 ));

...
- **05G — texture/SRV/sampler foundation (DONE / Windows signed off):** default-heap RGBA8 upload, shader-visible descriptors, explicit sampler binding and textured indexed geometry;
- **05H+ — visible real-caller migration (IN PROGRESS):** 05H1 opens `mingw64-game`, removes the full-runtime CMake gate, routes real untextured `W3DDisplay`/`Render2D` screen-space geometry plus gamma through `IRenderBackend`, and adds explicit D3D12 2D blend PSOs. Continue from actual `z_generals` blockers; next coherent slice is real `TextureClass` -> renderer-neutral texture handle integration for textured `Render2D`, then further boot-critical state/transform/draw callers until the normal x64 executable links/starts/renders without DX8;
